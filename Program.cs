...
calculator.AddRule("CLAMP", 4, 3 , values =>
{
...
calculator.AddRule("USD", 4, 1, values => (values[0] * 43.46) );
...
calculator.AddRule("g", 4, 0, values => 9.8);
// calculator.AddRule("t", 4, 0, values => 3 * calculator.Calculate("g"));
calculator.AddVariable("t", "3+g");
calculator.ShowVariables();

calculator.RemoveVariable("y");
// calculator.RemoveVariableAt(2);

Console.WriteLine("В КІНЦІ");
calculator.ShowVariables();
Console.WriteLine();
...
double trigonometric1 = calculator.Calculate("sin(30º) + cos(60º)"); // 1
double trigonometric2 = calculator.Calculate("tg(45º) - ctg(45º)");  // 0
double trigonometric3 = calculator.Calculate("sin(0) + cos(0)");   // 1
Console.WriteLine(calculator.Calculate("sin(30º)"));
Console.WriteLine(calculator.Calculate("sin 1"));
Console.WriteLine(calculator.Calculate("sin 30 + 1"));
...
double variables1 = calculator.Calculate("g");
double variables2 = calculator.Calculate("t");
Console.WriteLine($"'g' = {variables1} \n't' = {variables2}");
...
Console.WriteLine(calculator.Calculate("CLAMP(1 , 2, 100)"));
