...
public class RulesStorage
{
    ...
    public RulesStorage(bool defaultRules = true)
    {
        ...
        if (defaultRules)
        {
            ...
            AddRule("º", 4, 1, values => (values[0] * Math.PI)/ 180);
            AddRule("sin", 4, 1, args => Math.Sin(args[0]));
            AddRule("cos", 4, 1, args => Math.Cos(args[0]));
            AddRule("tg", 4, 1, args => Math.Tan(args[0]));
            AddRule("ctg", 4, 1, args => 1.0 / Math.Tan(args[0]));
